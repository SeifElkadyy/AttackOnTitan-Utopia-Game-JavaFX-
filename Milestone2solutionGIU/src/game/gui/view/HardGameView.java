...
public class HardGameView extends StackPane {
    ...
    public HardGameView(Stage primaryStage, Battle battle) {
        this.battle = battle;
    ...
    private void initializeUI(Stage primaryStage) {
        ...
        backButton.setOnAction(event -> {
            ...
            performSceneTransition(primaryStage, gameDifficultyView); // Perform transition
        });
...
//                weaponShopButton.setDisable(true); // Keep the weapon shop button disabled when AI is stopped
    ...
    private void updateLanes() {
        ...
        for (Lane lane : lanes) {
            ...
            StackPane.setMargin(healthProgressBar, new Insets(0, 0, 86, 1380));
            ...
            int[] weaponPositions = {900, 1000, 1100}; // Predefined positions for weapons
            int weaponIndex = 0; // Initialize weapon index
            for (Weapon weapon : lane.getWeapons()) {
                if (weaponIndex >= weaponPositions.length) {
                    break; // Ensure no more than three weapons per lane
                }
                ...
                StackPane.setMargin(weaponHBox, new Insets(0, 0, 0, weaponPositions[weaponIndex]));
                laneStackPane.getChildren().add(weaponHBox);

                weaponIndex++; // Increment the weapon index
            }
    ...
    private void returnToStartWindow(Stage primaryStage) {
        ...
        performSceneTransition(primaryStage, mainView); // Perform transition
    }
